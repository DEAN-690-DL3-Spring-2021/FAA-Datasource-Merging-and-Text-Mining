...
aid_data = read.csv('aid_stm.csv',colClasses = c("NULL","NULL",NA,NA,"NULL",NA,NA))
str(aid_data)

sum(aid_data$Phase_Of_Flight=='')

aid_data$Phase_Of_Flight[aid_data$Phase_Of_Flight=='']<-"Unknown"

identifier_aid <- read.csv('aid_stm.csv',colClasses = c("NULL",NA,"NULL","NULL","NULL","NULL","NULL"))
str(identifier_aid)
...
processed_aid <- textProcessor(aid_data$Remarks, stem= FALSE,onlycharacter = TRUE, wordLengths= c(4,Inf),
                               customstopwords=c('aircraft','airplane','pilot','flight','airport','found', 'airman','time','regulations','prevailed','approximately','without','faa','federal','stated','sustained','inspected','came','rest','plan','reported'),
                               metadata=aid_data)
...
stm_aid_15 <- stm(out$documents, out$vocab, K=15, prevalence=~Aircraft_Type+Phase_Of_Flight+Month_Of_Occurrence,
                       max.em.its=10, data=out$meta, init.type="Spectral",gamma.prior='L1',seed=1111)

stm_aid_10 <- stm(out$documents, out$vocab, K=10, prevalence=~Aircraft_Type+Phase_Of_Flight+Month_Of_Occurrence,
                  max.em.its=10, data=out$meta, init.type="Spectral",gamma.prior='L1',seed=1111)

stm_aid_20 <- stm(out$documents, out$vocab, K=20, prevalence=~Aircraft_Type+Phase_Of_Flight+Month_Of_Occurrence,
                  max.em.its=10, data=out$meta, init.type="Spectral",gamma.prior='L1',seed=1111)


plot(stm_aid)

plot(stm_aid, type="hist")

cloud_aid <- cloud(stm_aid)

topics_aid <- labelTopics(stm_aid,n=7)
...
tab_aid <- make.dt(model=stm_aid, meta = aid_data)
...
prep <- estimateEffect(1:15 ~Phase_Of_Flight+Month_Of_Occurrence, stm_aid, meta=out$meta, 
                       uncertainty="Global")
...
M10ExSem<-as.data.frame(cbind(c(1:10),exclusivity(stm_aid_10), semanticCoherence(model=stm_aid_10, docs_aid), "Mod10"))
...
td_theta <- tidytext::tidy(stm_aid_15, matrix = "theta")
...
td_beta <- tidytext::tidy(stm_aid_15)
